    ...
    @objc func bookMarkButtonTapped(_ sender: UIButton) {
        ...
        guard let bookData = bookData  else {
            ...
            if sender.isSelected {
                CoreDataManager.shared.createBookmark(bookData: bookData)
                print("Create")
            } else {
                let bookmark = CoreDataManager.shared.readBookmark()
                if let bookmarkToDelete = bookmark.first(where: { $0.title == bookData.documents[0].title }) {
